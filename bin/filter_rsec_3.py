...
class Tool:
    ...
    def __init__(self, folder_path: Path):
        ...
        # Combined, lower-cased description string for faster REGEX search
        self.combined_description_content = (
            self.descriptions.get('biocontainers', '') + ' ' + 
            self.descriptions.get('biotools', '') + ' ' + 
            self.descriptions.get('galaxy', '')
        ).strip().lower()
    ...
    def check_criteria_3(self) -> bool:
        """Check for keywords in combined descriptions."""
        if not self.combined_description_content: 
            return False
            
        # 1. Strict acronym checks (Word boundary search)
        for strict_ref_upper, pattern in zip(STRICT_KEYWORDS, COMPILED_STRICT_PATTERNS): 
            if pattern.search(self.combined_description_content):
                self.validation_data['biotools_description'] = strict_ref_upper
                return True 
                
        # 2. Regex checks (fragment patterns)
        for pattern in COMPILED_FRAGMENT_PATTERNS:
            match = pattern.search(self.combined_description_content)
            if match:
                self.validation_data['biotools_description'] = match.group(0)
                return True
...
# -------------------------------------------------------------
#                  Toolset class (Optimized and Silent)
# -------------------------------------------------------------
...
class ToolSet:
    ...
    def run_filtering(self):
        ...
        # Imprimer un saut de ligne après la boucle pour finaliser la progression
        sys.stdout.write('\n')
