...
class TestTool:
    ...
    def test_safe_load_reads_json_file(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
    ...
    def test_safe_load_reads_yaml_file(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
    ...
    def test_safe_load_returns_none_for_malformed_json(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
        p = tmp_path / "bad.json"
    ...
    def test_safe_load_returns_none_for_missing_file(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
    ...
    def test_load_all_metadata_detects_biotools_file(self, tmp_path):
    ...
    def test_load_all_metadata_detects_biocontainers_file(self, tmp_path):
    ...
    def test_load_all_metadata_detects_galaxy_file(self, tmp_path):
    ...
    def test_load_all_metadata_empty_folder_all_false(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
    ...
    def test_load_all_metadata_all_three_sources(self, tmp_path):
    ...
    def test_extract_edam_terms_returns_function_operations(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
    ...
    def test_extract_edam_terms_returns_topics(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
    ...
    def test_extract_edam_terms_returns_sorted_comma_separated(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
        data = {"topic": [{"term": "Zzz"}, {"term": "Aaa"}]}
        assert tool._extract_edam_terms(data, "topic") == "Aaa, Zzz"

    def test_extract_edam_terms_empty_data_returns_empty_string(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
        assert tool._extract_edam_terms({}, "function") == ""
        assert tool._extract_edam_terms({}, "topic") == ""

    def test_extract_edam_terms_deduplicates(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
    ...
    def test_store_full_report_metadata_fills_biotools_id(self, tmp_path):
        folder = _make_tool_folder(tmp_path, biotools=SAMPLE_BIOTOOLS)
        tool = _make_tool(folder)
        assert tool.validation_data["biotools_id"] == "mytool"

    def test_store_full_report_metadata_fills_edam_operations(self, tmp_path):
        folder = _make_tool_folder(tmp_path, biotools=SAMPLE_BIOTOOLS)
        tool = _make_tool(folder)
        assert "Sequence alignment" in tool.validation_data["EDAM_operations_full"]

    def test_store_full_report_metadata_fills_edam_topics(self, tmp_path):
        folder = _make_tool_folder(tmp_path, biotools=SAMPLE_BIOTOOLS)
        tool = _make_tool(folder)
        assert "Genomics" in tool.validation_data["EDAM_topics_full"]

    def test_store_full_report_metadata_empty_without_biotools(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
    ...
    def test_check_criteria_1_matches_on_topic(self, tmp_path):
        folder = _make_tool_folder(tmp_path, biotools=SAMPLE_BIOTOOLS)
    ...
    def test_check_criteria_1_matches_on_operation(self, tmp_path):
        folder = _make_tool_folder(tmp_path, biotools=SAMPLE_BIOTOOLS)
    ...
    def test_check_criteria_1_returns_false_when_no_match(self, tmp_path):
        folder = _make_tool_folder(tmp_path, biotools=SAMPLE_BIOTOOLS)
        tool = _make_tool(folder, target_topics=["Unknown"], target_ops=["Unknown op"])
        assert tool.check_criteria_1() is False

    def test_check_criteria_1_returns_false_without_biotools(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
        assert tool.check_criteria_1() is False

    def test_check_criteria_1_topic_takes_priority_over_operation(self, tmp_path):
        folder = _make_tool_folder(tmp_path, biotools=SAMPLE_BIOTOOLS)
        tool = _make_tool(
            folder,
    ...
    def test_check_criteria_2_matches_strict_keyword(self, tmp_path):
    ...
    def test_check_criteria_2_matches_fragment_pattern(self, tmp_path):
    ...
    def test_check_criteria_2_handles_keyword_string_with_semicolons(self, tmp_path):
        folder = _make_tool_folder(tmp_path, biocontainers={"keywords": "FASTA; alignment"})
        tool = _make_tool(folder, strict_kw=["FASTA"], frag_patterns=[])
        assert tool.check_criteria_2() is True

    def test_check_criteria_2_handles_keyword_string_with_commas(self, tmp_path):
        folder = _make_tool_folder(tmp_path, biocontainers={"keywords": "alignment,FASTA"})
        tool = _make_tool(folder, strict_kw=["FASTA"], frag_patterns=[])
        assert tool.check_criteria_2() is True

    def test_check_criteria_2_returns_false_when_no_match(self, tmp_path):
    ...
    def test_check_criteria_2_returns_false_without_biocontainers(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
        assert tool.check_criteria_2() is False
    ...
    def test_check_criteria_3_matches_strict_in_biotools_description(self, tmp_path):
    ...
    def test_check_criteria_3_matches_fragment_in_biocontainers_description(self, tmp_path):
    ...
    def test_check_criteria_3_matches_fragment_in_galaxy_description(self, tmp_path):
    ...
    def test_check_criteria_3_returns_false_when_no_match(self, tmp_path):
        folder = _make_tool_folder(tmp_path, biotools=SAMPLE_BIOTOOLS)
        tool = _make_tool(
            folder,
    ...
    def test_check_criteria_3_returns_false_with_all_empty_descriptions(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(folder)
        assert tool.check_criteria_3() is False

    def test_check_criteria_3_biotools_checked_before_galaxy(self, tmp_path):
    ...
    def test_run_checks_returns_true_and_sets_keep_on_criteria_1_match(self, tmp_path):
        folder = _make_tool_folder(tmp_path, biotools=SAMPLE_BIOTOOLS)
        tool = _make_tool(folder, target_topics=["Genomics"])
        assert tool.run_checks() is True
        assert tool.keep is True

    def test_run_checks_returns_true_and_sets_keep_on_criteria_2_match(self, tmp_path):
    ...
    def test_run_checks_returns_true_and_sets_keep_on_criteria_3_match(self, tmp_path):
    ...
    def test_run_checks_returns_false_and_sets_keep_false_when_all_fail(self, tmp_path):
        folder = _make_tool_folder(tmp_path)
        tool = _make_tool(
            folder,
            target_topics=[],
...
class TestToolSet:
    ...
    def test_init_sets_root_dir(self, tmp_path):
        ts = _make_toolset(tmp_path)
        assert ts.root_dir == tmp_path

    def test_init_output_dir_derived_from_json_out(self, tmp_path):
        ts = _make_toolset(tmp_path)
        assert ts.output_dir == tmp_path / "infos"

    def test_init_failed_json_out_in_output_dir(self, tmp_path):
        ts = _make_toolset(tmp_path)
        assert ts.failed_json_out.parent == ts.output_dir

    def test_init_report_txt_out_in_output_dir(self, tmp_path):
        ts = _make_toolset(tmp_path)
        assert ts.report_txt_out.parent == ts.output_dir

    def test_init_report_counts_all_zero(self, tmp_path):
        ts = _make_toolset(tmp_path)
        for v in ts.report_counts.values():
            assert v == 0

    def test_init_tools_list_empty(self, tmp_path):
        ts = _make_toolset(tmp_path)
        assert ts.tools == []

    def test_init_kw_data_loaded_from_file(self, tmp_path):
        ts = _make_toolset(tmp_path)
    ...
    def test_prepare_output_dir_creates_directory(self, tmp_path):
        ts = _make_toolset(tmp_path)
        assert not ts.output_dir.exists()
        ts._prepare_output_dir()
        assert ts.output_dir.is_dir()

    def test_prepare_output_dir_removes_existing_output_files(self, tmp_path):
        ts = _make_toolset(tmp_path)
    ...
    def test_prepare_output_dir_is_idempotent(self, tmp_path):
        ts = _make_toolset(tmp_path)
        ts._prepare_output_dir()
    ...
    def test_run_filtering_counts_all_tool_folders(self, tmp_path):
    ...
    def test_run_filtering_keeps_matching_tool_in_validated_json(self, tmp_path):
        kw_content = {
    ...
    def test_run_filtering_puts_rejected_tool_in_failed_json(self, tmp_path):
        kw_content = {
    ...
    def test_run_filtering_deletes_rejected_folders(self, tmp_path):
        kw_content = {
            "edam": {"operations": [], "topics": []},
            "keywords": [],
            "acronyms": [],
    ...
    def test_run_filtering_does_not_delete_output_dir(self, tmp_path):
        kw = _make_keywords_yaml(tmp_path)
        ts = _make_toolset(tmp_path, kw_file=kw)
        ts.run_filtering()
        assert ts.output_dir.is_dir()

    def test_run_filtering_increments_filter_1_counter_for_edam_match(self, tmp_path):
        kw_content = {
            "edam": {"operations": [], "topics": ["Genomics"]},
            "keywords": [],
            "acronyms": [],
    ...
    def test_write_json_creates_valid_json_file(self, tmp_path):
        ts = _make_toolset(tmp_path)
    ...
    def test_write_json_handles_empty_list(self, tmp_path):
        ts = _make_toolset(tmp_path)
    ...
    def test_finalize_deletes_listed_folders(self, tmp_path):
        ts = _make_toolset(tmp_path)
        ts._prepare_output_dir()
    ...
    def test_finalize_writes_report_file(self, tmp_path):
        ts = _make_toolset(tmp_path)
        ts._prepare_output_dir()
    ...
    def test_finalize_does_not_raise_if_folder_already_deleted(self, tmp_path):
        ts = _make_toolset(tmp_path)
        ts._prepare_output_dir()
    ...
    def test_write_report_contains_all_required_fields(self, tmp_path):
        ts = _make_toolset(tmp_path)
        ts.report_counts = {
            "total_folders": 20,
    ...
    def test_write_report_creates_file_if_missing(self, tmp_path):
        ts = _make_toolset(tmp_path)
...
class TestCloneRsecData:
    ...
    def _patch_run(self, return_value=True):
        return patch("utils.run_command_rsec", return_value=return_value)

    # ------------------------------------------------------------------ early-exit failure paths

    def test_returns_false_when_clone_command_fails(self, tmp_path):
    ...
    def test_returns_false_when_sparse_checkout_config_fails(self, tmp_path):
    ...
    def test_returns_false_when_checkout_command_fails(self, tmp_path):
    ...
    def test_returns_false_when_subdir_absent_after_checkout(self, tmp_path):
    ...
    def test_removes_existing_target_dir_before_cloning(self, tmp_path):
    ...
    def test_removes_leftover_temp_dir_before_cloning(self, tmp_path):
        temp_dir = tmp_path / "temp"
        temp_dir.mkdir()
    ...
    def test_writes_sparse_checkout_file_with_correct_content(self, tmp_path):
        temp_dir = tmp_path / "temp"
        temp_dir.mkdir()
    ...
    def test_returns_true_and_moves_subdir_to_target(self, tmp_path):
        temp_dir = tmp_path / "temp"
        temp_dir.mkdir()
        (temp_dir / ".git" / "info").mkdir(parents=True)
    ...
    def test_cleans_up_temp_dir_after_success(self, tmp_path):
        temp_dir = tmp_path / "temp"
        temp_dir.mkdir()
        (temp_dir / ".git" / "info").mkdir(parents=True)
        (temp_dir / "data").mkdir()
...
class TestLoadKeywordsFromYaml:
    ...
    def test_invalid_regex_in_keywords_is_skipped_with_warning(self, tmp_path, capsys):
        content = {
            "edam": {"operations": [], "topics": []},
    ...
    def test_empty_yaml_returns_all_empty_collections(self, tmp_path):
    ...
    def test_stricts_are_deduplicated(self, tmp_path):
        content = {
            "edam": {"operations": [], "topics": []},
            "keywords": [],
...
class TestGenerateTsvSummary:
    ...
    def test_tsv_has_expected_headers(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
        self._write_json(json_path, [{"tool_id": "t1"}])
        generate_tsv_summary(json_path, tsv_path)
    ...
    def test_tsv_row_count_matches_input(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
    ...
    def test_filtered_on_edam_topics(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
    ...
    def test_filtered_on_edam_operation(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
    ...
    def test_filtered_on_biocontainers_keywords(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
    ...
    def test_filtered_on_biotools_description(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
    ...
    def test_filtered_on_biocontainers_description(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
    ...
    def test_filtered_on_galaxy_description(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
    ...
    def test_unknown_reason_when_no_criteria_key_present(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
        self._write_json(json_path, [{"tool_id": "t1"}])
        generate_tsv_summary(json_path, tsv_path)
    ...
    def test_criteria_priority_edam_topics_over_operation(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
    ...
    def test_description_uses_biotools_first(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
        self._write_json(
            json_path,
    ...
    def test_description_falls_back_to_biocontainers(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
        self._write_json(
            json_path,
    ...
    def test_description_falls_back_to_na_when_all_empty(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
        self._write_json(json_path, [{"tool_id": "t1"}])
        generate_tsv_summary(json_path, tsv_path)
    ...
    def test_prints_error_and_returns_for_missing_json(self, tmp_path, capsys):
        generate_tsv_summary(tmp_path / "missing.json", tmp_path / "out.tsv")
        assert "ERROR" in capsys.readouterr().out

    def test_prints_error_and_returns_for_invalid_json(self, tmp_path, capsys):
        bad = tmp_path / "bad.json"
        bad.write_text("not json {{", encoding="utf-8")
        generate_tsv_summary(bad, tmp_path / "out.tsv")
        assert "ERROR" in capsys.readouterr().out

    def test_to_keep_column_always_true(self, tmp_path):
        json_path = tmp_path / "v.json"
        tsv_path = tmp_path / "out.tsv"
